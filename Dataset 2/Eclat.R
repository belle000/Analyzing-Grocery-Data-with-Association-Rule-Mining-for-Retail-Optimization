...
# Run eclat algorithm for both year
rules_2014 <- eclat(
  txn_2014, parameter = list(support = 0.002)
)
rules_2015 <- eclat(
  txn_2015, parameter = list(support = 0.002)
)

basket_rules_eclat_2014 <- ruleInduction(rules_2014, txn_2014, confidence = 0.5)
basket_rules_eclat_2015 <- ruleInduction(rules_2015, txn_2015, confidence = 0.5)

# Print number of rules and display the top rules sorted by support
cat("Number of rules for year 2014:", length(basket_rules_eclat_2014), "\n")
rules_sorted_by_lift_2014 <- sort(basket_rules_eclat_2014, by = "lift", decreasing = TRUE)
inspect(rules_sorted_by_lift_2014)  

cat("Number of rules for year 2015:", length(basket_rules_eclat_2015), "\n")
rules_sorted_by_lift_2015 <- sort(basket_rules_eclat_2015, by = "lift", decreasing = TRUE)
inspect(rules_sorted_by_lift_2015)  

# Plot rules 
plot(basket_rules_eclat_2014, measure = c("support", "confidence"), shading = "lift", main = "Eclat Rules for Year 2014")
plot(basket_rules_eclat_2015, measure = c("support", "confidence"), shading = "lift", main = "Eclat Rules for Year 2015")

plot(basket_rules_eclat_2014, method = "grouped", main = "Grouped Rules for 2014")
plot(basket_rules_eclat_2015, method = "grouped", main = "Grouped Rules for 2015")

plot(basket_rules_eclat_2014, method = "graph", main = "Graph of Rules for 2014")
plot(basket_rules_eclat_2015, method = "graph", main = "Graph of Rules for 2015")
